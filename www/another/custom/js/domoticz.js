...
Domotica.domoticz = {
	...
	update: function() {
		...
		that.call("devices", { plan: Domotica.settings.domoticz.plan }, function(data) {
			...
			$.each(data.result, function(idx, result) {
				...
				} else if(element.hasClass("usage")) {
					that.updateWidget.usage(element, result);
	...
	updateWidget: { 
		...
		usage: function(element, result) {                // 1-2 toegevoegd
        var usage = result.Usage ;
   
        // Update the usage itself
        element.find(".usage" ).html(usage);
